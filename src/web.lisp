...
(defclass booker-breach (booker-document)
...
(defclass booker-email (booker-document)
...
(defclass booker-email-message (booker-document)
...
(defclass booker-user (booker-document)
...
(defun new-email (email)
  ...
  (let* ((email-data (split-string email "@"))
         ...
         (email (make-instance 'booker-email :user user :domain domain)))
...
(defun new-email-with-password (user domain password)
  ...
  (let ((email (make-instance 'booker-email :user user :domain domain :password password)))
    (hash-id email (format nil "~a~a~a" user domain password))
    email))

(defun new-user (username platform &optional url)
  "Create a New Booker User"
  (let ((user (make-instance 'booker-user :name username :platform platform :url (or url "") :dtype "user")))
