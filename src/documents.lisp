...
(defclass booker-document ()
  ((id :accessor doc-id :type string :initarg :id)
   ...
   (date_updated :accessor doc-updated :type integer :initarg :date-updated)
   (date_added :accessor doc-updated :type integer :initarg :date-added)))


;; Not really needed but itsfor my own santiy from documents.nim
(defmethod make-uuid ((doc booker-document))
  "Create a UUID for the document"
  (setf (doc-id doc) (uuid:make-v4-uuid)))

(defmethod timestamp ((doc booker-document))
  "Add the current time in unix to the document"
  (setf (doc-added doc) (unix-now))
  (setf (doc-updated doc) (unix-now)))

(defmethod update-timetamp ((doc booker-document))
  "Update the doc_updated field to the current unix epoch time."
  (setf (doc-updated doc) (unix-now)))

(defmethod hash-id ((doc booker-document) data &optional (type :md5))
